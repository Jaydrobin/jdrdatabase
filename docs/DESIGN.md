...
3. 컬럼 타입 추론과 타입별 셀 렌더러: `string, number, date, boolean, list, tags, link, select, url`. `url` 셀은 값 전체를 링크로 렌더하고, `string` 셀은 텍스트 안에 섞인 `http(s)://` 주소만 링크로 바꿉니다. 셀 안의 링크 클릭은 행 선택이나 셀 편집을 시작하지 않습니다.
...
- 문자열 셀의 URL 자동 링크: 셀 텍스트는 먼저 HTML 이스케이프한 뒤 URL만 `<a>`로 감쌉니다. 링크 텍스트가 셀 너비를 넘으면 말줄임으로 자르고 전체 주소는 툴팁에 둡니다. URL 뒤에 붙은 마침표·쉼표·닫는 괄호는 링크에서 제외합니다.
...
1. `JDR.Notes` 구현(§4.9).
2. 편집 패널: 제목 입력(= 파일명), 프론트매터 폼(키·값 편집, 키 추가·삭제), 본문 `<textarea>`, 미리보기 토글. `<textarea>` 안에서 링크는 평문으로만 보이며 클릭은 미리보기에서 합니다. 편집 중에도 링크가 표시되길 원하면 ADR-12의 CodeMirror 교체가 그 경로입니다.
3. 링크 삽입 명령(`Ctrl+Shift+K`): 선택한 텍스트가 있으면 `[선택 텍스트](URL)`로 감싸고 URL 입력 상자를 띄웁니다. 선택이 없으면 `[](URL)`을 넣고 커서를 대괄호 안에 둡니다. 클립보드에 URL이 있으면 입력 상자의 기본값으로 채우고, 선택 텍스트 자체가 URL이면 입력 없이 `<URL>` 형태로 감쌉니다. 다른 노트로의 링크는 `[[`를 입력하면 노트 이름 자동 완성 목록이 뜨는 방식으로 넣습니다.
4. 자동 저장 2초 디바운스 + `Ctrl+S` 즉시 저장 + 저장 상태 표시.
5. `beforeunload`에서 `dirty`면 이탈 경고. 페이지 숨김(`visibilitychange: hidden`) 시 즉시 저장 시도.
6. 표 셀 편집: 타입별 편집기(텍스트, 숫자, 날짜 피커, 체크박스, 셀렉트, 태그 입력, URL 입력). `Enter` 확정, `Esc` 취소. 확정 시 `Notes.setField`.
7. 새 노트: 현재 뷰의 소스 폴더에 생성하고, 뷰 필터가 `eq` 조건이면 그 값을 프론트매터 기본값으로 채움(Projects와 동일한 동작).
8. 삭제는 항상 휴지통 이동(`trashed: true`). 영구 삭제는 제공하지 않음.

예외 처리:

- 링크 삽입 시 선택 텍스트에 `]`가 포함되거나 URL에 `)`나 공백이 포함됨: 텍스트의 `]`는 `\]`로, URL의 `)`와 공백은 퍼센트 인코딩으로 이스케이프해 마크다운 링크가 깨지지 않게 합니다.
- 링크 삽입 시 URL에 프로토콜이 없음(`example.com`): `https://`를 앞에 붙여 제안하되 사용자가 그대로 두면 상대 경로로 취급되므로 경고 표시.
...
4. 마크다운 렌더러: `marked`를 GFM 모드로 사용해 `[텍스트](URL)` 링크와 문법 없이 적은 `http(s)://` 주소를 모두 링크로 렌더. `marked` 확장으로 위키링크를 앱 내 링크로, 태그를 필터 링크로, `키:: 값`을 정의 목록으로 렌더. 렌더 결과는 `DOMPurify`로 정화.
5. 링크 열기 정책: 외부 링크는 새 탭에서 열고 `rel="noopener noreferrer"`를 붙입니다. 위키링크와 태그 링크는 같은 창에서 앱 상태만 바꿉니다. 정화 단계에서 `http`, `https`, `mailto` 외의 스킴은 제거하고, 외부 링크에는 도메인을 툴팁으로 표시합니다.
6. 백링크 패널과 "미해석 링크" 목록. 미해석 링크 클릭 시 해당 이름으로 새 노트 생성 제안.
7. 노트 간 이동 히스토리(뒤로/앞으로).
...
- 자동 링크 오인식: 코드 블록과 인라인 코드 안의 URL은 GFM 규칙대로 링크로 바꾸지 않습니다. `www.`로 시작하는 주소는 `https://`가 없어도 GFM 자동 링크 대상이므로 표의 문자열 셀 규칙과 달리 링크가 됩니다. 한글이나 괄호가 URL 끝에 붙은 경우 `marked`의 경계 판정을 따르고, 결과가 어긋나면 사용자가 `<URL>` 형태로 감싸 범위를 명시할 수 있습니다.
- 위키링크 안의 파이프와 마크다운 링크 충돌(`[[노트|별칭]]`이 표 셀 안에 있을 때): 표 셀에서는 `\|`로 이스케이프한 형태를 파이프로 되돌린 뒤 위키링크로 해석합니다.
