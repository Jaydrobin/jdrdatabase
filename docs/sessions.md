...
커밋: `docs(design)` v2 사용성 묶음 설계(`fc0eaa6`). 코드는 바꾸지 않았다.
...
- **진행 방식**: v2 묶음은 묶음마다 `main` 최신에서 새 브랜치·PR, N → O → P 순서로 앞 PR 병합 뒤 시작(`DESIGN.md` 5.0, `CLAUDE.md` 7.3·8). 세션 이름 J~M은 이미 쓰였으므로 N부터.
