...
- 6·7번을 푸시한 뒤의 CI `perf`·`check-build-e2e` 결과는 **미확인**입니다. 비교 항목이 10개로 줄고(바이트 3 + 처리량 7), 워크플로는 푸시당 한 번만 돕니다. 관측된 네 실행을 새 규칙으로 되돌려 본 결과는 전부 회귀 없음이지만, 실제 실행은 다음 푸시에서 확인합니다.
