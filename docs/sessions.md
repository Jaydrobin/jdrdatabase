...
초록인 상태에서 드러나지 않은 문제 4건을 재현 테스트와 함께 고쳤습니다. 새 기능은 없고 Step 9 이후를
앞당겨 구현하지 않았습니다.

시작 상태에 한 가지 짚을 것이 있습니다. 받은 직후 첫 `npm run check`가 단위 테스트 1건 실패로
끝났습니다(`# tests 297 # pass 296 # fail 1`). 그 출력의 꼬리만 보고 `not ok` 줄을 건지지 못해 어떤
테스트였는지 특정하지 못했습니다. 이어서 **세션 F 트리 그대로 136회**(단독 반복 65회, 4중·8중 병렬
28회, `npm run check` 3회, 그리고 편집 전까지 돌린 배경 반복 39회+1회)를 돌렸지만 한 번도 재현되지
않았고, 수정을 마친 트리에서 다시 50회를 돌린 것도 모두 초록입니다.

원인으로 의심할 만한 것을 먼저 지웠습니다. 러너(`scripts/run-unit-tests.mjs`)는 `--test-timeout`을
주지 않아 테스트별 시간 상한이 없고, 시간에 기대는 단언도 찾지 못했습니다(`createProgressReporter`
테스트는 `now`를 주입하고, `_updated_at` 테스트는 고정 상수 `NOW`·`LATER`를 씁니다. 가져오기의
`_created_at`은 `attempt()`가 한 번 찍은 값이라 `DISTINCT`가 항상 1입니다). 재현하지 못했으므로
아래 "미확인"에 남깁니다.

커밋: `7d7b941` → `7d09199` → `b12557d` → `b0ed0e0` (문제 하나당 커밋 하나).
...
- [x] `npm run check`: eslint 0건, prettier 통과, tsc 0오류, 단위 테스트 **302개 통과**(세션 F 297 +
      신규 5). 위에 적은 첫 실행 1건 실패는 재현하지 못했습니다(미확인).
...
- **받은 직후 첫 `npm run check`의 단위 테스트 1건 실패.** 같은 트리 134회 재실행에서 재현되지
  않았고 실패한 테스트를 특정하지 못했습니다. 다음 세션은 시작 `check`의 출력을 지우지 말고
  보관해 두면 좋겠습니다.
