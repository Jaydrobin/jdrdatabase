...
세션 H가 초록인 상태에서 드러나지 않은 문제 4건을 재현 테스트와 함께 고쳤습니다. 새 기능은 없고 세션 I(Step 11)를 앞당겨 구현하지 않았습니다.

커밋: `add6303` → `968103d` → `4ca05fb` → `3d082c4` (문제 하나당 커밋 하나).
...
- [x] `npm run check`: eslint 0건, prettier 통과, tsc 0오류, 단위 테스트 **348개 통과**(세션 H 346 + `engine-wasm.test.js` 1 + `report.test.js` 1).
- [x] `npm run build && npm run verify`: `verify OK`, 외부 참조 0건(허용 vendor URL 리터럴 160건), vendor 체크섬 6개 일치, 두 변형 CSP 외 동일.
- [x] `npm run test:e2e`: Chromium `file://`에서 **66개 통과**(65 + 위 1개).
- [ ] `npm run test:perf`: **미확인.** 1번은 `snapshot()` 진입의 `sqlite3_get_autocommit` 한 번, 2번은 휠 핸들러(렌더 경로 아님), 3번은 판정 코드 자체이고 측정에 닿지 않습니다. 30만 행 314 MB 픽스처 재생성 비용에 견주어 재측정하지 않았습니다. 3번의 판정 규칙은 단위 테스트가 기준선 실값으로 고정합니다.
...
- `npm run test:perf`를 이번 수정 뒤에 돌리지 않았습니다(위 검증 결과).
- 3번의 고친 판정 규칙이 실제 CI `perf` 잡에서 어떻게 나오는지는 다음 푸시의 실행으로 확인합니다. 기준선 자체는 건드리지 않았습니다.
