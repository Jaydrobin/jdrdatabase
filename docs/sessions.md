...
세션 H가 초록인 상태에서 드러나지 않은 문제 5건을 고쳤습니다(1~4번은 재현 테스트와 함께, 5번은 CI가 드러냈습니다). 새 기능은 없고 세션 I(Step 11)를 앞당겨 구현하지 않았습니다.

커밋: `add6303` → `968103d` → `4ca05fb` → `3d082c4` → (기록) → `5ff91c2` (문제 하나당 커밋 하나).
...
- 5번을 푸시한 뒤의 CI `perf` 결과는 **미확인**입니다. 비교 항목이 23개로 줄고 `grid.renderMaxMs`가 빠집니다. 세 실행(보정 비 1.33·1.51과 기준선 실행)에서 관찰한 잡음 원인은 기계 속도(3번)와 표본 하나짜리 바깥값(5번) 두 가지였고, 남은 시간 항목들은 세 실행 모두 보정된 기준 안이었습니다. 다음 세션들의 푸시에서 또 코드 변경 없이 빨강이 나오면 같은 식으로 그 항목의 잡음 원인을 적고 측정 방식을 고치는 것이지 기준선을 올리는 것이 아닙니다.
