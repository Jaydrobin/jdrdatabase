...
- **데스크톱 E2E**(`npm run test:desktop`): 이 컨테이너에 `tauri-driver`·`WebKitWebDriver`가 없어 돌리지 못했습니다(**미확인**). 데스크톱에서 체크를 모두 풀면 정리 실행 버튼이 꺼지는 것(10번)과 설정의 실패한 작업 사본 줄만 남는 것(8번)은 단위·native 테스트와 코드 경로로만 확인했습니다.
- **`cargo fmt`·`clippy`·`cargo test`**: 러스트 코드를 바꾸지 않아 돌리지 않았습니다(**미확인**, 변경 없음).
- **`npm run test:perf`**: 돌리지 않았습니다(**미확인**). 히스토리의 적용·되돌리기·다시 실행에 빈 목록 대기 한 번이 더해졌고 렌더·창 질의 경로는 바꾸지 않았습니다. CI `perf` 잡의 결과로 판단합니다.
