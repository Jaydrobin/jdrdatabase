...
커밋: `b89411f` docs(design) → `96fbeb6` refactor(ui) → `8807f57` feat(db) → `0a16f80` feat(grid) → `test(perf)`(빈 행 끝 점프) → 이 커밋 docs(session).
...
- 열 메뉴의 "오름차순·내림차순 정렬"은 그 열 하나만의 정렬로 바꾸고(정렬 버튼의 Shift 없는 클릭과 같음), "정렬 해제"는 그 열만 뺍니다. 설계에 없던 부분이라 적었습니다(**리뷰어 확인 필요**).
...
- "+ 열"의 이름 확정은 `column.add`와 `column.rename` 두 커맨드라 되돌리기 두 번에 열이 사라집니다(첫 번째는 자동 이름으로 되돌림). 기존 E2E "되돌리기·다시 실행: 셀 편집과 열 추가"를 이 동작에 맞춰 고쳤습니다(**리뷰어 확인 필요**: 한 번에 사라지게 하려면 추가·이름을 한 커맨드로 묶는 별도 op가 필요합니다).
...
- [x] E2E: 우클릭과 Shift+F10(그리고 ContextMenu 키)으로 연 열 메뉴에서 타입을 선택으로 바꾸고 예시(placeholder)·설명 줄(`aria-describedby`)이 보이는 입력칸에 항목 입력 → 확정. 정렬 버튼 클릭·Shift+클릭 순환 — `sheet.spec.js` 넷째 테스트. v1의 머리글 클릭 정렬 E2E(`view.spec.js` 3개)는 정렬 버튼 클릭으로 고쳤고, 머리글 이름을 눌러도 정렬되지 않는 단언을 더했습니다.
...
- [x] `npm run check`: 단위 **375개** 통과(365 + 이름 5, 테이블 2, 빈 행 확정 1, 스토어 1, 단축키 1). 중간 커밋 `96fbeb6`(365)·`8807f57`(372)도 각각 따로 check 초록.
- [x] `npm run build` → `npm run verify`: 통과. `dist/jdrdatabase.html` **3,879,035 bytes**(설계 v2 기록 3,860,804에서 **+18,231 bytes**: 머리글·메뉴·빈 행 코드와 두 언어 문구). 외부 참조 0, vendor 체크섬 OK.
- [x] `npm run test:e2e`: **77개** 통과(70 − 고친 것 없음 + `sheet.spec.js` 6 + `a11y.spec.js` 1).
- [x] `npm run test:perf`: 7개 중 6개 통과. **`search.perf.spec.js`의 LIKE가 1,083 ms로 로컬 예산(1초)을 넘었습니다.** 이 PR의 회귀가 아닌 근거: 같은 컨테이너에서 기준 커밋 `ca22c21`과 이 브랜치를 번갈아 두 번씩 돌린 A/B에서 기준 커밋도 1,167·1,101 ms, 이 브랜치 1,053·1,116 ms였습니다(보정값 776~834 ms로 이 컨테이너가 이날 느림). 이 항목은 세션 E·H부터 "예산 경계"(959~998 ms)로 기록된 것이고, 이 세션은 질의 경로를 바꾸지 않았습니다. CI는 보정된 기준선 비교로 판정합니다.
- [x] `npm run test:native`: **29개** 통과(실제 rusqlite 엔진, Rust 1.94로 이 컨테이너에서 빌드). 데스크톱 코드는 바꾸지 않았지만 `tables.create`의 배치 단계가 데스크톱 모드에서 `run_batch`로 가므로 돌렸습니다. `test:desktop`(tauri-driver)은 돌리지 않았습니다(**미확인**).
...
- LIKE 검색 로컬 예산 초과: 기준 커밋도 같은 값이라 이 세션의 회귀는 아니지만, 8장 예산 경계 항목으로 **이 컨테이너에서 예산을 넘는 상태**입니다. 이 푸시의 CI `perf` 결과는 **미확인**(푸시 뒤 확인).
