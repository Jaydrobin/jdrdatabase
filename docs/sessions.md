...
- 잡음 바닥을 더한 커밋 `e414ef0`의 CI(run 35614563258)는 `search.likeShortMs: 967.9 > 716.3 × 1.3 + 5`로 실패했습니다. 같은 코드로 713·716·968 ms가 나오는 항목이라(로컬도 959~998) 한 번 재는 값으로는 30% 규칙이 흔들립니다. 서로 다른 무일치 두 글자 검색어 3개의 중앙값으로 재도록 바꾸고(행 수 캐시가 같은 검색어를 다시 재지 않으므로), 그 항목의 기준선은 비운 뒤 다음 CI 실행 값으로 채웁니다(후속 커밋). 그 뒤 CI가 "회귀 없음"으로 끝나는지는 미확인. 다른 23개 항목은 두 실행 모두 30% + 잡음 바닥 안이었습니다.
