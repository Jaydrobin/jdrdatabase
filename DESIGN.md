...
Step은 설계·검증의 단위이고, 세션은 구현·리뷰의 단위다. Step 0~11을 아래 5.0의 세션 A~I로 묶어 순서대로 진행하며, 세션 하나는 PR 하나로 끝난다. 묶음에 속한 모든 Step의 "완료 기준"이 충족되어야 다음 세션으로 간다. 단계 안의 세부 순서는 위에서 아래로 진행한다.
...
세션 절차:
- 시작: `CLAUDE.md` 2장 순서로 문서를 읽고, 기준 브랜치에서 `npm run check`(세션 I는 `cargo test` 포함)가 초록인지 확인한다. 직전 세션의 PR 설명에 "미확인"으로 남은 항목을 이어받는다.
- 종료: 묶음에 속한 모든 Step의 완료 기준을 PR 설명에 항목별로 옮겨 적고 검증 결과를 표시한다. 검증하지 못한 항목은 "미확인"으로 남긴다.
- 세션 도중 컨텍스트가 부족해지면 완료된 Step까지만 PR로 올리고, 남은 Step은 같은 세션 이름의 후속 PR(예: B-2)로 이어 간다. 완료 기준을 낮추어 끝내지 않는다.
