...
**산출물**: `db/cleanup.js`, `db/schema.js`(임시 테이블 이름 `_jdr_tmp_<id>`), `db/engine-wasm.js`·`db/engine-native.js`·`db/engine.js`(`capabilities().compactsOnSave`, `vacuum()`), `db/worker.js`·`db/client.js`(`cleanup.plan`·`cleanup.run`), `app/store.js`(`planCleanup`·`runCleanup`·`discardAllWorkcopies`·`clearBackups`), `io/idb.js`(`keys`), `ui/dialogs/cleanup.js`, `ui/dialogs/settings.js`, `ui/dialogs/dialog.js`(본문에서 버튼 제출), `app/history.js`(`cleanup:done`), `i18n/ko.js`·`en.js`(`column.delete.message`를 정리 위치로 고침), `docs/desktop.md`·`docs/cloud-sync.md`(정리 뒤 저장해야 파일이 줄어듦, 남는 사본), 테스트
...
- `cleanup.run(engine, { columns: [{ tableId, columnId }] }, ctx)` → `{ cmds, removedColumns, rebuiltIndexes, vacuumed, vacuumError, bytesBefore, bytesAfter }`. 순서: (1) 요청한 열이 모두 소프트 삭제 상태인지 검사, (2) 바깥 트랜잭션 안에서 테이블마다 `purgeCommand(target, columnIds)`를 만들어 `applyCommand`, (3) 커밋, (4) `compactsOnSave`가 `false`면 `engine.vacuum()`. `vacuumError`는 `VACUUM`이 실패했을 때의 직렬화된 오류이고 아니면 null이다(재작성은 이미 커밋되었으므로 던지지 않고 결과로 알린다). `bytesBefore`·`bytesAfter`는 `page_count × page_size`.
...
- `ui/dialogs/cleanup.js`: 테이블별 삭제된 열 목록(모두 체크), DB 크기와 줄일 수 있는 빈 공간, 되돌릴 수 없음과 히스토리가 비워짐, 남는 사본(D-17), "파일은 저장해야 작아진다". 실행 중 진행률·취소(v1 내보내기 대화상자처럼 확인 버튼의 비동기 `validate`로 돌리고, 실행 중의 취소 버튼은 신호만 당긴다). `dbBytes × 2`가 `capabilities().maxFileBytes`를 넘으면 브라우저 메모리가 모자랄 수 있다는 경고를 실행 전에 보인다(실행은 허용).
...
| `cleanup.plan` | | `{ tables, dbBytes, freeBytes, compactsOnSave }`(Step 13 `cleanup.plan`). `tables[i] = { tableId, name, columns: [{ id, name, type, deletedAt }], ftsEnabled }`는 소프트 삭제된 열이 있는 STRICT 테이블. 읽기 op | |
| `cleanup.run` | `{ columns: [{ tableId, columnId }] }`. 비어 있으면 빈 공간 줄이기만 | `{ cmds, removedColumns, rebuiltIndexes, vacuumed, vacuumError, bytesBefore, bytesAfter }`. `cmds`는 테이블마다 되돌릴 수 없는 `column.purge` 커맨드이며 메인이 저널에 넣고 히스토리를 비운다. 진행 이벤트 `{ phase: 'purge' \| 'index' \| 'vacuum', done, total }`. 배타 op(D-17) | 가능(재작성·인덱싱 중. 전체 롤백, `E_IMPORT_CANCELLED`). `VACUUM`은 불가 |
