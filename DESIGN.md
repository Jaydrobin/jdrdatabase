...
위 표의 예산은 로컬 측정(`npm run test:perf`)의 판정 기준이다. CI 러너는 환경이 다르고 흔들리므로 절대 예산 대신 `test/perf/perf-baseline.json`(같은 러너에서 잰 기준선) 대비 30% 이상 회귀를 실패로 본다(CLAUDE.md 6장). 1~2 ms짜리 항목(셀 편집 왕복)은 타이머 해상도만으로 30%가 흔들리므로 비율에 잡음 바닥(ms 항목 5 ms, bytes 항목 16 MiB)을 더해 둘 다 넘어야 회귀다. 또한 GitHub 러너는 실행마다 다른 기계에 배정되어 같은 코드가 1.4배쯤 흔들리므로(세션 H 실측), 실행 시작 때 같은 wasm 엔진으로 고정 작업(`test/perf/calibrate.js`: 10만 행 삽입 + LIKE 5회)을 재어 기준선을 잴 때의 값과의 비([0.5, 2]로 자름)만큼 기대값을 늘리거나 줄인다. 기준선은 `calibrationMs`와 측정값을 같은 실행에서 통째로 옮겨 적으며, 갱신은 원인과 함께 PR 설명에 적는다.
