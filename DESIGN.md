...
위 표의 예산은 로컬 측정(`npm run test:perf`)의 판정 기준이다. CI 러너는 환경이 다르고 흔들리므로 절대 예산 대신 `test/perf/perf-baseline.json`(같은 러너에서 잰 기준선) 대비 30% 이상 회귀를 실패로 본다(CLAUDE.md 6장). 1~2 ms짜리 항목(셀 편집 왕복)은 타이머 해상도만으로 30%가 흔들리므로 비율에 잡음 바닥(ms 항목 5 ms, bytes 항목 16 MiB)을 더해 둘 다 넘어야 회귀다. 기준선 갱신은 원인과 함께 PR 설명에 적는다.
