...
- 추가와 이름 합치기: "+ 열" 직후 열린 이름 편집기(`grid.startRename`)의 확정은 `mergeWithAdd`로 기록되고, 스토어가 커맨드 알림에 그 표시를 실어 보낸다. 히스토리는 되돌리기 스택의 맨 위가 같은 테이블의 `column.add`이고 다시 실행 스택이 비어 있을 때만 두 커맨드를 `commands.mergeCommands(add, rename)`(`do`는 add → rename, `undo`는 rename의 undo → add의 undo, 한 트랜잭션)로 바꿔 한 항목으로 둔다. 조건이 맞지 않으면 따로 쌓는다. 저널에는 두 커맨드가 이미 차례로 기록되어 있고, 합친 항목을 되돌리면 그 역커맨드가 기록되므로 재생 결과가 같다. 머리글 더블클릭·메뉴·사이드바의 이름 바꾸기는 합치지 않는다. 자동 이름의 `taken`은 그 테이블의 살아 있는 열과 소프트 삭제된 열의 이름 전부다. 새 테이블의 기본 열 수 `NEW_TABLE_COLUMNS = 30`은 `ui/sidebar.js`의 상수다.
...
- 빈 행 확정(`ui/grid/editing.js`): 인라인·장문 편집기, 불리언 체크, 붙여넣기가 빈 행 `i`(`i >= count`)를 대상으로 하면 확정 시점의 `query.stats`로 `count`와 `maxId`를 읽어 새 id `maxId + 1 … maxId + (i − count + 1)`을 정하고(`commands.ghostRowInserts`) `commands.bulkEdit({ tableId, edits: [], inserts, now })` 하나로 적용한다. 마지막 행만 값을 가지고 나머지는 빈 행이다. 빈 값을 확정하면 만들 것이 없으므로 커맨드 없이 닫는다. 붙여넣기는 v1의 "범위를 넘으면 행을 자동 추가" 경로에 시작 위치가 빈 행인 경우를 더한다: `clipboard.planPaste`가 마지막 실제 행과 시작 행 사이의 빈 줄 수(`gapRows`)를 함께 돌려주고, 그 줄은 빈 행으로 만든다. 되돌리기 한 번에 만들어진 행이 모두 사라진다. 장문 편집기는 빈 행에서 `rowId: null`로 빈 값을 열고, 확정은 대상의 `commit`(위 커맨드)이 맡는다.
...
- 선택·이동: `selection.setBounds`는 실제 행 + 빈 행을 받으므로 화살표·PageDown으로 빈 행에 들어간다. `Ctrl+End`는 마지막 실제 행으로 간다.
- 빈 행이 꺼지는 전환은 그리드가 편집 컨트롤러의 `hooks.onGhostDisabled()`로 알린다(같은 테이블의 다시 마운트에서 정렬·필터·검색이 켜질 때, 스토어의 `state:changed`로 읽기 전용이 될 때). 컨트롤러는 빈 행에 열려 있던 인라인·장문 편집기를 닫고 `grid.ghostClosed`를 알린다.
