...
- 스토어: `store.createTable(name, { columns })`(만든 테이블을 고른다), `store.addDefaultColumn(tableId)` → `{ columnId, columnCount }`(자동 이름 + `text`로 `schema.addColumn`), `store.renameColumn(tableId, columnId, name)`(머리글 이름 편집기의 확정). 자동 이름의 `taken`은 그 테이블의 살아 있는 열과 소프트 삭제된 열의 이름 전부다. 새 테이블의 기본 열 수 `NEW_TABLE_COLUMNS = 30`은 `ui/sidebar.js`의 상수다.
...
- `menu.open(anchor, items, { label?, returnFocus?, onClose? })`: `items[i] = { key, label, disabled?, danger?, run }`. `role="menu"`/`menuitem`, ↑↓·Home·End로 이동, Enter·Space로 실행, Esc·Tab·바깥 클릭으로 닫고 호출한 곳(`returnFocus`, 열 메뉴는 그리드 스크롤 영역)으로 포커스를 돌려준다. 열 메뉴 항목은 D-16의 순서이며 "타입 변경…"은 v1의 `promptChangeType`, "열 삭제…"는 `confirmDeleteColumn`을 그대로 쓴다(사이드바와 같은 경로를 `ui/dialogs/column.js`의 `changeColumnTypeFlow`·`deleteColumnFlow`로 공유한다). "오름차순·내림차순 정렬"은 그 열 하나만의 정렬로 바꾸고(정렬 버튼의 Shift 없는 클릭과 같다), "정렬 해제"는 정렬 목록에서 그 열만 뺀다(그 열이 정렬에 없으면 꺼진다).
