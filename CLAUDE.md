...
- v1(A~I)은 작업 브랜치 하나와 그 PR 하나에서 진행했다. v2 묶음(N~P)은 묶음마다 `main` 최신에서 새 작업 브랜치와 PR을 만들고, 앞 묶음의 PR이 병합된 뒤에 다음 묶음을 시작한다(`DESIGN.md` 5.0). 세션은 `DESIGN.md` 5.0의 묶음 하나를 맡아 Step 단위 커밋으로 푸시하며, 푸시 전에 7.1 체크리스트를 전부 확인한다. 컨텍스트 부족으로 묶음을 끝내지 못하면 완료된 Step까지만 푸시하고 후속 세션(예: B-2)으로 이어 간다.
...
- v1 PR은 세션 I까지 끝난 뒤에, v2 묶음의 PR은 그 묶음이 끝난 뒤에 병합한다. 조건은 CI 초록, 리뷰어 1명 승인, `docs/sessions.md`의 해당 절에서 미확인 항목이 해소된 것이다.
- 리팩터링과 기능 추가를 한 커밋에 섞지 않는다.

## 8. Git 규약

- 브랜치: v1 구현은 PR이 열린 작업 브랜치 하나를 모든 세션이 공유했다. v2 묶음은 묶음마다 새 브랜치를 만들고, 같은 묶음의 후속 세션(예: N-2)만 그 브랜치를 이어 쓴다. 병합 뒤의 수정은 `fix/<요약>`, `docs/<요약>`. 원격 실행 환경이 작업 브랜치 이름을 정해 주면 그 이름을 쓴다. 기본 브랜치 `main`에 직접 푸시하지 않는다.
