...
서버 없이 브라우저에서 단독 동작하는 스프레드시트형 SQLite 데이터베이스 관리 앱. 배포 단위는 `dist/jdrdatabase.html` 파일 하나. 엔진은 sql.js(SQLite WASM), UI는 프레임워크 없는 Vanilla JS, 저장은 표준 SQLite 파일.

## 2. 작업 시작 전 필독

1. `DESIGN.md` 2장(핵심 설계 결정 D-01 ~ D-14). 결정과 어긋나는 코드는 리뷰에서 반려된다.
2. `DESIGN.md` 5장에서 지금 진행 중인 Step의 "주요 함수 / 예외 처리 / 완료 기준".
3. 이 파일 전체.
...
Node.js 20 이상. 런타임 npm 의존성은 없다. `devDependencies`만 허용된다.
...
| 경로 | 규칙 |
|---|---|
| `src/db/*` | Worker에서 실행되는 코드. DOM, `window`, `document`에 접근하지 않는다 |
| `src/import/*`, `src/export/*` | Worker에서 실행. `DOMParser` 등 메인 스레드 전용 API 금지 |
| `src/ui/*`, `src/app/*`, `src/io/*` | 메인 스레드. sql.js를 직접 호출하지 않고 `db/client.js`만 사용한다 |
...
- `db.export()` 뒤에는 prepared statement가 모두 무효화된다. statement를 모듈 변수에 캐시하지 말고 `engine.prepareCached()`만 쓴다.
...
- 런타임 서드파티는 sql.js와 SheetJS CE 두 개뿐이다. 추가하려면 `DESIGN.md` D-12를 먼저 고친다.
...
- [ ] 진행 중인 Step의 "완료 기준"을 PR 설명에 항목별로 옮겨 적고 각각 어떻게 확인했는지 적음
...
5. **Worker 경계**: 직렬화 불가 값, DOM 접근, 메인 전용 API.
...
- CI 초록, 리뷰어 1명 승인, 7.1 체크리스트 전부 확인.
- 한 PR은 한 Step(또는 그 일부) 범위를 넘지 않는다. 리팩터링과 기능 추가를 한 PR에 섞지 않는다.

## 8. Git 규약

- 브랜치: `feat/step-<n>-<요약>`, `fix/<요약>`, `docs/<요약>`. 기본 브랜치 `main`에 직접 푸시하지 않는다.
...
- 작업 범위는 요청된 Step 또는 이슈로 한정한다. 다음 Step을 앞당겨 구현하지 않는다.
...
- 브라우저 API 가용성(`file://`에서의 Worker, IndexedDB, File System Access)은 추정으로 단정하지 않는다. 실측 결과를 `docs/support-matrix.md`에 기록하고, 확인하지 못한 항목은 "미확인"으로 남긴다.
