...
- 구현은 작업 브랜치 하나와 그 PR 하나에서 진행한다. v1(A~I)과 v2(N~P)가 각각 그렇게 하며, v2는 `claude/busy-mayer-hv2g8x`를 쓴다(`DESIGN.md` 5.0). 세션은 `DESIGN.md` 5.0의 묶음 하나를 맡아 Step 단위 커밋으로 푸시하며, 푸시 전에 7.1 체크리스트를 전부 확인한다. 컨텍스트 부족으로 묶음을 끝내지 못하면 완료된 Step까지만 푸시하고 후속 세션(예: B-2)으로 이어 간다.
...
- PR 병합은 v1은 세션 I, v2는 세션 P까지 끝나고 CI 초록, 리뷰어 1명 승인, `docs/sessions.md`의 해당 절에서 미확인 항목이 모두 해소된 뒤에 한다.
- 리팩터링과 기능 추가를 한 커밋에 섞지 않는다.

## 8. Git 규약

- 브랜치: v1·v2 구현은 각각 PR이 열린 작업 브랜치 하나를 그 버전의 모든 세션이 공유한다. 세션마다 새 브랜치를 만들지 않는다. 원격 실행 환경이 다른 작업 브랜치를 정해 주면 코드를 쓰기 전에 사용자에게 확인한다. 병합 뒤의 수정은 `fix/<요약>`, `docs/<요약>`. 기본 브랜치 `main`에 직접 푸시하지 않는다.
