...
- [ ] 세션 묶음에 속한 모든 Step의 "완료 기준"을 PR 설명에 항목별로 옮겨 적고 각각 어떻게 확인했는지 적음. 확인하지 못한 항목은 "미확인"으로 표시
...
- 한 PR은 `DESIGN.md` 5.0의 세션 묶음 하나(A~I)를 넘지 않는다. 묶음 안의 Step은 커밋으로 나눈다. 컨텍스트 부족으로 묶음을 끝내지 못하면 완료된 Step까지만 PR로 올리고 후속 PR(예: B-2)로 이어 간다.
- 리팩터링과 기능 추가를 한 PR에 섞지 않는다.

## 8. Git 규약

- 브랜치: `feat/session-<a~i>-<요약>`(예: `feat/session-b-file-schema`), `fix/<요약>`, `docs/<요약>`. 기본 브랜치 `main`에 직접 푸시하지 않는다.
...
- 세션 시작 시 기준 브랜치에서 `npm run check`가 초록인지 확인하고, 직전 세션 PR 설명의 "미확인" 항목을 이어받는다. 세션 종료 시 PR 설명에 검증 결과와 미확인 항목을 분리해 적는다.
