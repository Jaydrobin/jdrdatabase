...
- [ ] PR 설명에 이번 세션 이름의 절을 추가하고, 묶음에 속한 모든 Step의 "완료 기준"을 항목별로 옮겨 적어 각각 어떻게 확인했는지 적음. 확인하지 못한 항목은 "미확인"으로 표시
...
- 세션별 검증 기록은 PR 설명에 세션 이름의 절로 쌓는다. 세션이 끝난 뒤 그 절이 없거나 미확인 항목이 표시되지 않은 푸시는 되돌린다.
- PR 병합은 세션 I까지 끝나고 CI 초록, 리뷰어 1명 승인, 모든 세션 절의 미확인 항목이 해소된 뒤에 한다.
...
- 세션 시작 시 작업 브랜치를 원격 최신으로 맞추고 그 상태에서 `npm run check`가 초록인지 확인한다. 초록이 아니면 직전 세션의 문제이므로 새 Step을 시작하기 전에 먼저 고친다. 직전 세션 PR 설명의 "미확인" 항목을 이어받는다. 세션 종료 시 PR 설명에 세션 이름의 절을 추가해 검증 결과와 미확인 항목을 분리해 적는다.
